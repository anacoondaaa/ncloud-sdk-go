...
var _ = Describe("Stop Server Instances", func() {
	Describe("Stop Server Instances", func() {
		BeforeEach(func() {
			gock.New("https://api.ncloud.com").
				Get("/server").
				Reply(200).BodyString(`<startServerInstancesResponse>
		...
		It("should start server instances", func() {
			reqParams := new(RequestStartServerInstances)
			reqParams.ServerInstanceNoList = []string{"551227"}

			conn := NewConnection(accessKey, secretKey)
			result, err := conn.StartServerInstances(reqParams)
	...
	Describe("The input parameter server instance number is required", func() {
		It("should be fail with no parameters", func() {
			...
			conn := NewConnection(accessKey, secretKey)
			_, err := conn.StartServerInstances(reqParams)
