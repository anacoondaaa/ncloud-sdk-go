...
var _ = Describe("Stop Server Instances", func() {
	...
	Describe("The input parameter server instance number is required", func() {
		It("should be fail with no parameters", func() {
			...
			serverConn := NewServerConnection(accessKey, secretKey)
			_, err := serverConn.StopServerInstances(reqParams)
