...
import (
	...
	. "github.com/NaverCloudPlatform/ncloud-sdk-go/server"
...
var _ = Describe("Get Block Storage Instance List", func() {
	Describe("Get all Block Storage Instance List", func() {
		BeforeEach(func() {
			gock.New("https://api.ncloud.com").
				Get("/server").
				Reply(200).BodyString(`<getBlockStorageInstanceListResponse>
		...
		It("should get Block Storage Instance list", func() {
			serverConn := NewServerConnection(accessKey, secretKey)
			result, err := serverConn.GetBlockStorageInstance(nil)
	...
	Describe("Get One Block Storage Instance List which ServerInstanceNoList.1 is 320897", func() {
		...
		It("should get One Block Storage Instance", func() {
			...
			serverConn := NewServerConnection(accessKey, secretKey)
			result, err := serverConn.GetBlockStorageInstance(reqParams)
	...
	Describe("Authorize fail", func() {
		...
		It("should be failed by autorization fail", func() {
			serverConn := NewServerConnection(accessKey, secretKey)
			result, err := serverConn.GetServerInstanceList(nil)
